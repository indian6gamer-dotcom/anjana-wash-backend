...
class services_collection:
    ...
    async def insert_one(self, doc):
        ...
        values = [1 if v is True else (0 if v is False else v) for v in doc.values()]
    ...
    async def insert_many(self, docs):
        ...
        placeholders = ", ".join(["?"] * len(docs[0]))
        
        values_list = []
        for doc in docs:
            values_list.append([1 if v is True else (0 if v is False else v) for v in doc.values()])
    ...
    async def update_one(self, filter, update):
        ...
        for k, v in update.get("$set", {}).items():
            ...
            set_params.append(1 if v is True else (0 if v is False else v))
...
class config_collection:
    ...
    async def find_one(self, filter, projection=None):
        conn = self._connect()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM config WHERE id = ?", (filter.get("_id", "pins"),))
        row = cursor.fetchone()
        conn.close()
        if row:
    ...
    async def insert_one(self, doc):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR IGNORE INTO config (id, worker_pin, owner_pin) VALUES (?, ?, ?)",
    ...
    async def update_one(self, filter, update):
        ...
        query = f"UPDATE config SET {', '.join(set_clauses)} WHERE id = ?"
        cursor.execute(query, params)
        conn.commit()
        conn.close()
...
class SQLiteDB:
    def __init__(self, db_path):
        self.db_path = db_path
        self.services = services_collection(db_path)
        self.bookings = bookings_collection(db_path)
        self.config = config_collection(db_path)
        self.counters = counters_collection(db_path)
        self._init_tables()
    ...
    def _init_tables(self):
        ...
        # Check if booking_source column exists in bookings table, if not add it
        cursor.execute("PRAGMA table_info(bookings)")
        cols = [r[1] for r in cursor.fetchall()]
        if "booking_source" not in cols:
            cursor.execute("ALTER TABLE bookings ADD COLUMN booking_source TEXT")
