...
const formatDateLabel = (dateStr) => {
  ...
  return `${prefix}${parseInt(dd, 10)} ${mName} ${yyyy}`;
};
...
export default function Owner() {
  ...
  const load = useCallback(async () => {
    try {
      const pin = sessionStorage.getItem("anjana_owner_pin") || "9999";
      const [a, b] = await Promise.all([
        api.get(`/bookings/stats/today?pin=${pin}`),
        api.get(`/bookings?pin=${pin}`)
      ]);
      setStats(a.data || { total_earnings: 0, cash_amount: 0, online_amount: 0, total_washes: 0, daily_breakdown: [] });
      setBookings(b.data || []);
    } catch (e) {
      console.error("Owner page load error:", e);
      setStats({ total_earnings: 0, cash_amount: 0, online_amount: 0, total_washes: 0, daily_breakdown: [] });
      setBookings([]);
    }
  }, []);

  const handleMarkPaid = async (bookingId) => {
    const pin = prompt("Enter Owner PIN to approve this payment:");
    if (!pin) return;
    try {
      await api.post(`/bookings/${bookingId}/mark-paid`, { owner_pin: pin });
      toast.success("Payment marked as paid successfully! Vehicle added to worker queue.");
      load();
    } catch (e) {
  ...
  const dynamicCompletedCount = completedForStats.length;
  const dynamicPendingCount = pendingForStats.length;
  ...
  const chartData = [
    { name: "Cash", value: dynamicCashAmount, fill: "hsl(var(--accent))" },
    { name: "Online", value: dynamicOnlineAmount, fill: "hsl(var(--primary))" },
  ];
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8" data-testid="owner-dashboard">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="label-caps">Control Room · {isAll ? stats.date : selectedDate}</div>
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3">
        <Stat testid="stat-earnings" icon={IndianRupee} label={`${labelSuffix}'s Earnings`} value={`₹${dynamicTotalEarnings}`} highlight />
        <Stat testid="stat-cash" icon={Banknote} label={`${labelSuffix}'s Cash`} value={`₹${dynamicCashAmount}`} sub={`${dynamicCashCount} txn`} />
        <Stat testid="stat-online" icon={Smartphone} label={`${labelSuffix}'s Online`} value={`₹${dynamicOnlineAmount}`} sub={`${dynamicOnlineCount} txn`} />
        <Stat testid="stat-completed" icon={CircleCheck} label={`${labelSuffix}'s Completed`} value={dynamicCompletedCount} sub={`${dynamicPendingCount} pending`} />
      </div>
      ...
      <div className="mt-6 grid md:grid-cols-3 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="relative rounded-[18px] overflow-hidden p-6 bg-secondary text-secondary-foreground flex flex-col justify-between elev-3 noise">
          ...
          <div className="relative">
            ...
            <div className="font-display font-black text-6xl tracking-tighter mt-1">{dynamicTotalBookings}</div>
          </div>
          <div className="relative text-sm opacity-80">{labelSuffix}'s footfall at Anjana Wash.</div>
        </motion.div>
      </div>
          ...
          {groupedCompletions.map((group) => (
            <div key={group.date} className="space-y-3">
              <div className="flex items-center gap-3 px-1 pt-2">
                <span className="font-display font-black text-xs tracking-wider text-primary uppercase bg-primary/10 px-3 py-1 rounded-full">
              ...
              <div className="grid gap-3">
                {group.items.map((b, idx) => (
                  <motion.div
                    ...
                    transition={{ delay: idx * 0.01, duration: 0.35 }}
                  ...
                  >
                    <img src={`${BACKEND_URL}/api/bookings/${b.id}/photo`} alt="" className="h-16 w-24 object-cover rounded-[10px] border border-border flex-shrink-0" />
...
function ServiceManager({ onClose }) {
  ...
  const refresh = async () => {
    const pin = sessionStorage.getItem("anjana_owner_pin") || "";
    const [s, c] = await Promise.all([api.get(`/owner/services?owner_pin=${pin}`), api.get("/categories")]);
